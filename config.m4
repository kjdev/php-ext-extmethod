dnl config.m4 for extension extmethod
...
PHP_ARG_ENABLE(extmethod, whether to enable extmethod support,
[  --enable-extmethod           Enable extmethod support])

if test "$PHP_EXTMETHOD" != "no"; then
  PHP_NEW_EXTENSION(extmethod, extmethod.c, $ext_shared)
fi
